...
use crate::utils::quality_mapper::{audio_quality_to_description, video_quality_to_description};
    ...
    if let Some(support_formats) = data.get("support_formats").and_then(|v| v.as_array()) {
        for format in support_formats {
            if let Some(quality) = format.get("quality").and_then(|q| q.as_i64()) {
                ...
                // 判断权限要求
                let (vip_only, login_required) = get_quality_requirements(quality_code);
...
/// 获取默认的视频质量选项
pub fn get_default_quality_options() -> Vec<QualityOption> {
    vec![
        QualityOption {
            quality: 127,
            description: "8K 超高清".to_string(),
            available: false,
            vip_only: true,
        ...
        QualityOption {
            quality: 116,
            description: "1080P 60帧".to_string(),
            available: false,
            vip_only: true,
            login_required: false,
        },
        QualityOption {
            quality: 112,
            description: "1080P 高码率".to_string(),
            available: false,
            vip_only: true,
            login_required: false,
        },
        QualityOption {
            quality: 80,
            description: "1080P 高清".to_string(),
            available: false,
            vip_only: false,
            login_required: true,
        },
...
/// 获取默认的音频质量选项
pub fn get_default_audio_quality_options() -> Vec<AudioQualityOption> {
    vec![
        AudioQualityOption {
            quality: 30251,
